...
public class WebSecurityConfig {
    ...
    public WebSecurityCustomizer webSecurityCustomizer(){
        ...
        //webconfig에 설정한 addResourceHandler는 정적자원에 대해 요청을 할수있게 해주는 역할
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                //restapi혹은 jwe기반 인증에서는 세션을 사용하지 않아서 보호를 하지 않아도됨
                .csrf(AbstractHttpConfigurer::disable)

                //어플리케이션의 session상태를 비저장 모드로 동작하게함
                .sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))

                //기존 formlogin을 사용하지 않으므로 비활성화
                .formLogin(AbstractHttpConfigurer::disable)

                //http기본인증(jwt를 사용할것이므로)비활성화
                .httpBasic(AbstractHttpConfigurer::disable)

                //사용자가 입력한 아이디 패스워드를 전달받아 로그인을 직접적으로 수행하는 필터
                //인증서 (sucesshandler를 통해)토큰을 생성해서 header로 전달하고
                //실패시 (failureHandler를 통해)실패이유를 담아서 응답한다
